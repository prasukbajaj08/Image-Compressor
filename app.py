...
# ----------functions-----------------
    ...
    def select_file(self):
        fileName, _ = QFileDialog.getOpenFileName(self,"QFileDialog.getOpenFileName()", "","All Files (*);; JPEG (*.jpeg);; PNG (*.png)")
        if fileName:
            print(fileName, _)
    ...
    def select_folder_source(self):
        ...
        except (PIL.UnidentifiedImageError, OSError, IndexError):
            # Handle UnidentifiedImageError, OSError (e.g., if the file is not an image),
            # or IndexError (if the 'files' list is empty)
            print("Skipping non-image file or empty folder.")
            return
    ...
    def resize_pic(self):
        ...
        if okPressed and new_pic_name != '':
            ...
            # # Resize the image (replace 100 with your desired width)
            # resized_img = img.resize((int(self.quality_path.text()), img.height))

            # # Save the resized image
            # resized_img.save(new_pic)

            # print(f"Resized image saved at: {new_pic}")
